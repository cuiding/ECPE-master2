...
#embedding_dim：词向量维度 embedding_dim_pos：位置嵌入维度
#load_w2v(FLAGS.embedding_dim, FLAGS.embedding_dim_pos, 'data_combine/clause_keywords.csv', FLAGS.w2v_file)
def load_w2v(embedding_dim, embedding_dim_pos, train_file_path, embedding_path):
    print('\nload embedding...')

    words = []
    inputFile1 = open(train_file_path,encoding='utf-8')
    #inputFile1 = open(train_file_path,'rb')
...
def load_data(input_file, word_idx, max_doc_len = 75, max_sen_len = 45):
    ...
    n_cut = 0
    inputFile = open(input_file,encoding='utf-8')
    while True:
...
def prf_2nd_step(pair_id_all, pair_id, pred_y, fold = 0, save_dir = ''):
    ...
    def write_log():
        ...
        g = open(save_dir+'pair_log_fold{}.txt'.format(fold))
